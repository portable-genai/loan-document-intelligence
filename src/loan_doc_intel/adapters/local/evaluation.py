...
credential-free heuristic gate that guards CI (extraction accuracy, validation
recall/precision, PII safety). SDK-free and unconditional. This proves the A4 promotion
gate runs entirely off-cloud.
"""
...
# local -> adapters -> loan_doc_intel -> src -> <repo>.
...
class LocalOfflineEvalAdapter:
    ...
    def _load_run_eval():  # type: ignore[no-untyped-def]
        if not _EVAL_SCRIPT.exists():
            return None
        spec = importlib.util.spec_from_file_location("loan_doc_local_run_eval", _EVAL_SCRIPT)
        if spec is None or spec.loader is None:
            return None
        module = importlib.util.module_from_spec(spec)
        sys.modules.setdefault("loan_doc_local_run_eval", module)
