...
allows benign input and BLOCKS on prompt-injection / jailbreak patterns (e.g. "ignore all
previous instructions", "exfiltrate", "system prompt"). This deterministically blocks a
malicious input and allows the benign one, so the blocked-path behaviour is driven by the
input text rather than by swapping in a special fake. There is no Google emulator for
Model Armor, so this path is unconditional.
"""
