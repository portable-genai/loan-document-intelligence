"""Local session adapter (SessionPort) : in-process per-application conversation state.

The ``local`` profile's stand-in for **Agent Platform Sessions**: a small in-process
store of sessions and their message history, seedable and deterministic. Under ``local``
this platform client uses an in-process implementation rather than HTTP to a sibling
service (a laptop runs one app, not the whole platform). When the Firestore emulator is
opted in (``FIRESTORE_EMULATOR_HOST`` set AND the client lib imports), the adapter routes
to it instead; the google client is imported lazily, only on that branch, so the default
...
class LocalSessionAdapter:
    ...
    def create_session(self, user_id: str, application_id: str | None = None) -> Session:
        self._n += 1
        sid = f"sess-{self._n}"
        session = Session(id=sid, user_id=user_id, application_id=application_id)
        if self._fs is not None:
            self._fs.collection("sessions").document(sid).set(
                {"id": sid, "user_id": user_id, "application_id": application_id}
            )
