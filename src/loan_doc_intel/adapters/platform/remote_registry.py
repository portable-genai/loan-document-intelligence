...
In the full platform deployment, B5's A2A AgentCard is published to and resolved through
the shared ``agent-registry`` service (A2A + MCP catalog) rather than served only from
B5's own ``/.well-known/agent-card.json``. This adapter implements
:class:`AgentRegistryPort` against the registry's ``/v1/agents`` endpoints (A3 contract):
...
class RemoteRegistryError(LoanDocError):
...
class RemoteRegistryAdapter:
    ...
    def _parse_card(body: dict) -> AgentCard:
        ...
        return AgentCard(
            ...
            skills=skills,
            provider=str(body.get("provider", "loan-document-intelligence")),
        )
