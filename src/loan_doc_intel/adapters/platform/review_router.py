...
from ``HUMAN_REVIEW_URL`` and the S2S credentials reuse this repo's shared platform S2S env vars
(``S2S_TOKEN`` / ``S2S_SIGNING_KEY``, sourced from ``adapters/platform/_s2s``), set on the Cloud Run
/ platform service. No cloud SDK is involved (the kit uses stdlib ``urllib`` plus the
wire-compatible S2S headers), so this module imports cleanly with no GCP SDK; it is bound under both
the ``platform`` and ``gcp`` profiles because it makes a real network call to a sibling service.
"""
