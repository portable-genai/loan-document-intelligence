...
FinOps) instead of B5 calling Cloud Logging directly. This adapter implements
:class:`AuditSinkPort` by POSTing the already-redacted :class:`AuditEvent` to the
observability service's ``/v1/audit`` endpoint, which returns ``202 Accepted`` (A5
contract).
...
class RemoteAuditError(LoanDocError):
