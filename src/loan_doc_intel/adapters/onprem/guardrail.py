...
port binds to the Model Armor adapter; switching ``profile`` to ``onprem`` rebinds it
...
parity. ``screen`` deliberately raises rather than fail-opening: an unimplemented
guardrail must never silently allow traffic, so porting on-premise *must* provide a real
screening backend. Filling this body in is the only change required.
"""
