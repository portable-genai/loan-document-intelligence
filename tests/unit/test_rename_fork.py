...
def _diverging_args() -> Namespace:
    ...
    return Namespace(
        package="acme_income_agent",
        cli="acme-income",
        env_prefix="ACME",
        resource="acme-income-agent",
        dist="acme-income-dist",
    )
...
def test_dist_cli_and_resource_each_land_on_their_own_value() -> None:
    ...
    assert rewritten == (
        'name = "acme-income-dist"\n'
        'acme-income = "acme_income_agent.cli.main:app"\n'
        'resource_id = "acme-income-agent"\n'
        "ACME_PROFILE\n"
    )
    assert count == 4
