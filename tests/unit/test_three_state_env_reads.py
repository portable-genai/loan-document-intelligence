...
TWO_STATE_READS_WITH_A_REASON: dict[str, str] = {
    "PORT": (
        "src/loan_doc_intel/api/app.py: the TCP port the local uvicorn runner listens on, "
        "supplied by the container runtime (Cloud Run injects it). It names no host, origin, "
        "audience, credential or profile, it grants nothing, and the bind ADDRESS, which is the "
        "part that carries posture, is decided separately by resolve_bind_host. An emptied "
        "value taking the documented 8092 changes only which port a process nobody can reach "
        "differently listens on."
    ),
}
