...
Rewrites the package name, the CLI entry point, the ``LOAN_DOC_`` env-var prefix, and the
...
In this repo the distribution name, the CLI name and the resource-id stem are the SAME
string (``loan-document-intelligence``), so ``--dist`` defaults to ``--resource`` and a fork normally
sets all three to one new value. Pass ``--dist`` only if you deliberately want a
distribution name that differs from the resource stem.
...
wiring, region) that this script does NOT do for you.
"""
...
_OLD_PACKAGE = "loan_doc_intel"
_OLD_CLI = "loan-document-intelligence"
_OLD_ENV_PREFIX = "LOAN_DOC_"
_OLD_RESOURCE = "loan-document-intelligence"
_OLD_DIST = "loan-document-intelligence"
...
def _rewrite_text(text: str, repls: list[tuple[str, str]]) -> tuple[str, int]:
    ...
    for old, new in repls:
        # For the env prefix, only rewrite ``LOAN_DOC_`` when it prefixes an UPPER token
        # (LOAN_DOC_PROFILE), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument(
        "--resource", required=True, help="new resource id stem, e.g. acme-income-agent"
    )
    ...
    print("Then finish the human steps in docs/ADOPTING.md (fixtures, IdP, region).")
    return 0
