...
def _metrics_block() -> list[str]:
    ...
    lines += ["", f"Scored over {len(cases)} golden applications.", ""]
    return lines
...
def _exercised_block() -> list[str]:
    cases = load_jsonl(GOLDEN)
    fields = sum(
        len(names) for row in cases for names in (row.get("expected_fields") or {}).values()
    )
    scored = sum(1 for row in cases if row.get("expected_fields"))
    inconsistent = sum(1 for row in cases if row.get("expected_failed_checks"))
    planted = sum(1 for row in cases if row.get("pii_in_inputs"))
    return [
        BLOCKS[1],
        "",
        f"- **{len(cases)} golden applications** in `eval/datasets/golden_cases.jsonl`, of which",
        f"  **{inconsistent} carry planted inconsistencies** for the validator to catch and the",
        "  rest are consistent, which is what a false positive can fire on.",
        f"- **{fields} reviewer-named fields across {scored} cases**, which is what",
