...
title: `loan-document-intelligence` named GCP deployment dossier
description: Non-secret decisions and evidence required before claiming a hosted `loan-document-intelligence` onboarding demonstration.
status: draft
---

# `loan-document-intelligence` named GCP deployment dossier
...
| Portal parent origin | PENDING | exact `journey-portal` HTTPS origin in frame policy |
...
5. Prove IAP, health profile/region, the `loan-document-intelligence` artifact, citations, review escalation and audit
   correlation through `journey-portal`'s hosted RM route.
