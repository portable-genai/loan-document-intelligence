...
| **R5** | Pass the **Hrz4 eval gate** before promotion | Promotion blocked unless `EvalReport.passed`; the `platform` profile delegates to Hrz4 `/v1/evaluations`, and CI runs the in-repo offline gate | `EvaluationGatePort`, `adapters/platform/remote_evaluation.py`, `eval/run_eval.py`, the hosted GitHub Actions check |
